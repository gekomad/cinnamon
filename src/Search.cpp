...
template<uchar side>
int Search::qsearch(int alpha, const int beta, const uchar promotionPiece, const int depth, const int npieces) {
    ...
    // TODO provare if (score > alpha) alpha = score;
    ...
    while ((move = getNextMoveQ(&genList[listId], first++))) {
        ...
        /// **************Delta Pruning ****************
        if (npieces >= 6 && score + PIECES_VALUE[move->capturedPiece] + 300 <= alpha &&
        ...
        /// ************ end Delta Pruning *************
        int val = -qsearch<X(side)>(-beta, -alpha, move->promotionPiece, depth - 1, npieces - 1);
        score = max(score, val);
        takeback(move, oldKey, oldEnpassant, false);
        if (score > alpha) {
            if (score >= beta) {

...
_Tmove *
GenMoves::getNextMove(_TmoveP *list, const int depth, const u64 &hash, const int first, const _Tmove *previousMove) {
    ...
    //    bad capture
    // .|........|..|...............|.           |......
    // T|  cap   |kl|    history    |countermove |bad capture
    // 1| 7 bits |2b|   15 bits     |            |6 bits
    ...
    for (int i = first; i < list->size; i++) {
        ...
        if (move.type & STANDARD_MOVE_MASK) {
            ...
            score |= isCounterMove(previousMove, move) << 6;
